...
namespace GameUnitTest
{
    ...
    public class CommentServiceUnitTest
    {
        ...
        public void AddCommentToGame()
        {
            ...
            commentService.Newcomment((int) game.Key, newCommentFirst);
            commentService.Newcomment((int) game.Key, newCommentSecond);
        ...
        public void AddCommentToComment()
        {
            ...
            commentService.Newcomment((int) commentService.GameComments("001").FirstOrDefault().CommentId, newCommentSecond);
            //Assert
        ...
        public void GameComments_NullTest()
        {
            var comments = commentService.GameComments(null);
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentException))]
        public void GameComments_WrongKeyTest()
        {
            var comments = commentService.GameComments("testId");
        }

        [TestMethod]
        public void newcommentMethodTest()
        {
            ...
            var parentComment = commentService.GameComments(gameKey).FirstOrDefault();

            int startCount = parentComment.Comments.Count;
            var childComment = new Comment();

            commentService.Newcomment(parentComment, childComment, gameKey);

            int resultCount = commentService.Get(parentComment.CommentId).Comments.Count;

            Assert.AreEqual(startCount + 1,resultCount);
        }
        ...
        public void GetComment_Test()
        {
            ...
            Assert.AreEqual(comment.CommentId, commentId);
        }

        [TestMethod]
        public void GetComment_WronIdTest()
        {
            var comment = commentService.Get(-100);

            Assert.IsNull(comment);
        }

        [TestMethod]
        public void BanComment_ID_Test()
        {
            int commentId = 1;
            ...
            Assert.AreEqual("Удалено модератором!", comment.Body);
        }

        [TestMethod]
        [ExpectedException(typeof(ArgumentException))]
        public void BanComment_ID_WrongIDTest()
        {
            commentService.Ban(-100);
        }

        [TestMethod]
        public void BanComment_Comment_Test()
        {
            var comment = commentService.Get(1);

            commentService.Ban(comment);
        ...
        [TestMethod]
        [ExpectedException(typeof(ArgumentException))]
        public void BanComment_Comment_WrongCommentTest()
        {
            commentService.Ban(new Comment());
        }
         * */
    }
}
